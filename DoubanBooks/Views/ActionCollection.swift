...
class ActionCollection: UIView {
    ...
    fileprivate func addTableView(_ count: Int){
       ...
       //   let table = UITableView(frame: CGRect(x: 20, y: y, width: UIScreen.main.bounds.width - 40, height: CGFloat(rowHeight * n)))
        let mzz = UICollectionView(frame: CGRect(x: 20, y: y, width: UIScreen.main.bounds.width - 40, height: CGFloat(rowHeight * n)))
    ...
    init(title: String,count: Int,dataSoure: UICollectionViewDataSource,delegate: UICollectionViewDelegate) {
    ...
    func show(superView: UIView) ->ActionCollection{
